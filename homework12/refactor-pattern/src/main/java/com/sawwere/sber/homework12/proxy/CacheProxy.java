...
import com.sawwere.sber.homework12.storage.InMemoryCacheStorage;
import com.sawwere.sber.homework12.storage.file.FileCacheStorage;
...
public class CacheProxy implements InvocationHandler {
    ...
    private CacheProxy(Object delegate, CacheProxyConfig config) {
        ...
        fileCacheStorage = new FileCacheStorage(config.getCacheDir(), config.getShouldCleanOnExit());
    }
    ...
    private Object innerInvoke(Cache cacheParams, Method method, Object[] args) {
        try {
            ...
            if (invokeResult instanceof List<?> list) {
                invokeResult = list.subList(0, Math.min(cacheParams.maxListSize(), list.size()));
            }
            return invokeResult;
        } catch (Throwable e) {
